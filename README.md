...
- Fast, JSON-first extraction when available (JSON-LD or embedded API responses).
...
1. JSON-first: attempts to extract detail URLs and listing data from JSON-LD or embedded JSON APIs.
2. Sitemap/ItemList: parses ItemList / CollectionPage JSON-LD when present to get canonical listing URLs.
3. HTML link discovery: falls back to collecting listing links from page anchors and listing cards.
4. Detail parsing: visits each listing page (if enabled) and extracts structured fields.
...
- Rate limiting or blocking: enable `proxyConfiguration`, lower `maxConcurrency` in code if needed, or add retries. Zameen.com may block without proxies.
